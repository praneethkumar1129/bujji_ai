  ...
  async function loadHistory() {
    try {
      ...
      data.chats.forEach(chat => {
        ...
        div.textContent = chat.message.substring(0, 40) + "...";
        div.onclick = () => {
          chatBox.innerHTML = "";
          append("user", chat.message);
          renderStructuredReply(chat.response);
        };
  ...
  function send() {
    ...
    if (isTranslate) {
      ...
      fetch("/api/chat", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: txt, username })
      })
      .then(res => res.json())
      .then(data => {
        document.getElementById("chat-loader")?.remove();
      ...
      .catch(err => {
        document.getElementById("chat-loader")?.remove();
        append("bujji", "❌ Chat error: " + err.message);
        console.error(err);
      });
    }
  }
